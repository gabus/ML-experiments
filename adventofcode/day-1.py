...
def do(input: list[str]) -> int:
	...
	for i in input:
		...
		for index, word in enumerate(spelled_numbers):
			for found_index in re.finditer(word, i):
				i = i[:found_index.start() + 1] + str(index + 1) + i[found_index.start() + 2:]

		s = re.findall("\d", i)

		math = 0
		if len(s) > 1:
			math = int(str(s[0]) + str(s[-1]))
		elif len(s) == 1:
			math = int(str(s[0]) + str(s[0]))
...
assert do(['1abc2', 'pqr3stu8vwx', 'a1b2c3d4e5f', 'treb7uchet']) == 142
assert do(['1abc2four', 'two']) == 36
assert do(['1abc2', 'asdfsas', 'two']) == 34
assert do(['adfsd', 'asdfsas']) == 0
assert do(['two1nine', 'eightwothree', 'abcone2threexyz', 'xtwone3four', '4nineeightseven2', 'zoneight234',
           '7pqrstsixteen']) == 281
assert do(['eightwothree']) == 83
assert do(['fiveight2']) == 52
assert do(['fiveight']) == 58
assert do(['fiveight', '1']) == 69
